...
{
    public class ArchibobaseExperienciaController : ApiController
    {
        ...
        public ArchibobaseExperienciaController()
        {
            fileDafult = Convert.ToBase64String(File.ReadAllBytes(HttpContext.Current.Server.MapPath("~/Image") + "\\no-file.png"));
        ...
        public async Task<Experiencia> ValidarSubirArchivosAsync(bool multiParse, string path = "", string extension = "")
        {
            if (!multiParse)
            {
